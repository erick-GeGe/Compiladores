...
// estructura que almacenara las produciones dadas en la entrada
struct AugmentedProduction
{
    char lhs;   // lado izquierdo
    string rhs; // lado derecho

    AugmentedProduction() {}
    AugmentedProduction(char _lhs, string _rhs) : lhs(_lhs), rhs(_rhs) {}
};

// Clase que representara los elementos en LR0
class LR0Item
{
private:
...
public:
    ...
    // retorna el resultado de si esta produccion ya esta contenida en la proStr de la produccion
    ...
    // Sobrecargar el operador para mayor flexibilidad
...
/**
 * void load_grammar
 * Se encarga de cargar la entrada y de guardar datos necesario */
...
int main() {
    int itemid = -1; // contador de elementos LR(0)
    AugmentedGrammar grammar;
    vector<LR0Item> lr0items = { LR0Item() }; // agregando estado inicial
    GotoMap globalGoto;
